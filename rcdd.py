...
class RCDDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for node_type in tqdm(nodes['node_type'].unique(), desc='Processing node info...'):
            ...
            x = np.vstack([np.asarray(f.split(':'), dtype=np.float32) for f in subset['node_feat']])
        ...
        for edge_type in tqdm(edges['edge_type'].unique(), desc='Processing edge info...'):
            ...
            data[src_type, edge_type, dst_type].edge_index = torch.stack([src, dst], dim=0)
